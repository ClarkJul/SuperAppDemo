...
public class CircularProgressButton extends View {
    ...
    public static final int DOWLOAD_STATE=0;

    //    定义 宽 高 半径 开始角度 滑动角度
    private int mW, mH, mRadius, mStartAngle = 0, mSweepAngle = 0;

    //    默认弧形的颜色，进度条颜色，暂停杠杠的颜色，字体颜色
    private int mDefColor,mRectColor, mProgressColor, mTabColor, mFontColor;

    //   画圆角矩形，画进度弧度，画中间暂停斜杠，画默认弧度，画文字
    private Paint mPaintForRect, mPaintForCircle, mPaintForTag, mPaintForDefCircle, mPaintForText;
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        mDefColor = typedArray.getColor(R.styleable.CircularProgressButton_defColor, Color.parseColor("#9E9E9E"));
        mRectColor = typedArray.getColor(R.styleable.CircularProgressButton_rectColor, Color.parseColor("#9E9E9E"));
        mProgressColor = typedArray.getColor(R.styleable.CircularProgressButton_progressColor, Color.parseColor("#FF34A350"));
        mTabColor = typedArray.getColor(R.styleable.CircularProgressButton_tabColor, Color.parseColor("#FF34A350"));
        mFontColor = typedArray.getColor(R.styleable.CircularProgressButton_fontColor, Color.parseColor("#FF34A350"));


        mStrokeWidthCircle = typedArray.getDimension(R.styleable.CircularProgressButton_progressWidth, 12);
        mRectWidth = typedArray.getDimension(R.styleable.CircularProgressButton_rectWidth, 6);

        mTipsStr = typedArray.getString(R.styleable.CircularProgressButton_tipsStr);
        if (TextUtils.isEmpty(mTipsStr)) {
            mTipsStr = "下载";
        }
        mTipsInstallStr = typedArray.getString(R.styleable.CircularProgressButton_finishStr);
        if (TextUtils.isEmpty(mTipsInstallStr)) {
            mTipsInstallStr = "安装";
        }
        mTipsStrFontSize = typedArray.getDimension(R.styleable.CircularProgressButton_fontSize, 40);
        typedArray.recycle();

        //画圆角矩形
        mPaintForRect = new Paint(Paint.ANTI_ALIAS_FLAG);
        mPaintForRect.setStyle(Paint.Style.FILL);
    ...
    private void drawRoundRect(Canvas canvas) {
        mPaintForRect.setStrokeWidth(mRectWidth);
        mPaintForRect.setColor(mRectColor);
        RectF rectF = new RectF(-mRadius * mMultiple, -mRadius, mRadius * mMultiple, mRadius);
        canvas.drawRoundRect(rectF, 60, 60, mPaintForRect);
//        mPaintForRect.setStrokeWidth(mStrokeWidthCircle);
     ...
     * @param colorRgb
     */
    public void setRectColor(int colorRgb) {
        mRectColor = colorRgb;
    }
